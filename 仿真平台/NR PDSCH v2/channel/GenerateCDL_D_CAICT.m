...
function [H] = GenerateCDL_D_CAICT(BlockInd,channel)
%% wavelength
    ...
    for s = 1 : BSAntennaTotalNumber
        ...
        for iCluster =  1 : ClusterNum
            ...
            if LOS == 1 && iCluster == ClusterNum
                ...
                tmp(:) = sqrt(CDL.power(iCluster))*F_rx*[1 0;0 -1]*F_tx*exp(1j*k_CONST*rx*du')*exp(1j*k_CONST*tx*ds')...
                    *exp(1j*(k_CONST*rx*VDirection*t ))*exp(-1j*k_CONST*d_3D);
            else
                tmp = zeros(20,P_num);
                for isubpath = 1 : 20
