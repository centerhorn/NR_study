...
%% code
function [SignalOut, SignalInterfere] = PassChannel(SignalIn,PreInterfere,H,channel)
...
%% 信号过信道
SignalTemp = [];
Hbar = zeros(UE_ANT_NUM,NB_ANT_NUM,MulPath,N);
SignalInTemp = [];
SignalInTemp2 = zeros(NB_ANT_NUM,N+MAX_DELAY);
for n=1:MulPath
    DelayAdd = delayout(n);
    Hbar(:,:,n,(DelayAdd+1):(DelayAdd+N))=H(:,:,n,(DelayAdd+1):(DelayAdd+N));
    SignalInTemp2(:,(DelayAdd+1):(DelayAdd+N)) = SignalIn;
    SignalInTemp = [SignalInTemp;SignalInTemp2];
end
SignalTemp2 = reshape(SignalInTemp,NB_ANT_NUM,MulPath,[]);
for u=1:UE_ANT_NUM %注意这是接收端天线
    HTemp=squeeze(Hbar(u,:,:,:));
    SignalTemp3 = SignalTemp2.*HTemp;
    SignalTemp = [SignalTemp; squeeze(sum(SignalTemp3,2))];
end
...
%         HTemp=squeeze(Hbar(u,s,:,:));
%         SignalTemp2 = SignalInTemp(s:NB_ANT_NUM:end,:).*HTemp;
%         SignalTemp((u-1)*NB_ANT_NUM+s,:) = sum(SignalTemp2,1);
%     end
% end
...
%% 输出信号
SignalOutTemp = SignalTemp(:,1:N);
SignalInterfere = SignalTemp(:,N+1:N+PreSeqNum);
%% 信号合并
if channel.NBAntNum==1 && channel.UEAntNum==1
    SignalOut = SignalOutTemp;
else
    SignalOut = [];
    for RxAntNum = 1:channel.UEAntNum
        SignalOut = [SignalOut;sum(SignalOutTemp((RxAntNum-1)*channel.NBAntNum+1:RxAntNum*channel.NBAntNum,:))];          
    end
end
