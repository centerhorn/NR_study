...
%% Input
%  DMRSMapSet:   cell array(RANK 1-4)
%                each cell:3-Dimensional array (subcarrier-symbol-rank).
%                          DMRS sequence mapping for all layers.
%  PDCCH_LOCATON: row vector
%                 PDCCH location
%% Output
%  RSMapMatrixSet  cell array(RANK 1-4)
%                  each cell:3-dimensional array (subcarrier-symbol-rank).
...
%%
function RSMapMatrixSet = nrPDSCHRSMapMatrix(DMRSMapSet, PDCCH_LOCATION)
RANKMax =4;
RSMapMatrixSet = cell(1,RANKMax);
for i = 1:RANKMax
    DMRSMap = DMRSMapSet{i};
