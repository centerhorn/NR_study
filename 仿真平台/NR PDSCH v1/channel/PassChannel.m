...
for u=1:NB_ANT_NUM %注意这是接收端天线
    for s=1:UE_ANT_NUM %发送天线
        for n=1:MulPath
            DelayAdd = delayout(n);
            HTemp=H(u,s,n,:);
...
if channel.NBAntNum==1 && channel.UEAntNum==1
    ...
    for RxAntNum = 1:channel.UEAntNum
        SignalOut = [SignalOut;sum(SignalOutTemp((RxAntNum-1)*channel.NBAntNum+1:RxAntNum*channel.NBAntNum,:))];           %%此处是否有错？？？
    end
end
