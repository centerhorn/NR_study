...
%  PHDSCH DMRS mapping to physical resources (refer to 3GPP TS38.211 Sec.7.4.1.1)
%% Input
%  gnbConfig：Structure.
...
%  CDMGroupMap：1-dimensional array.
...
%%
function [DMRSMatrix, CDMGroupMap]= nrPDSCHDmrsMatrix(gnbConfig, pdschConfig)  
...
elseif PDSCH_DMRS_CONFIG_TYPE == 'type 2'    % refer to Table 7.4.1.1.2-2
 ...
 PortNum = length(port);
 DMRSMatrixAll = zeros(SubcarrierNum,SYMBOLS_PER_SLOT,PortNum);
DMRSMatrix = zeros(SubcarrierNum,SYMBOLS_PER_SLOT,RANK);
...
for lBarInd = 1:lBarLen
    for lPrimeInd = 1:lPrimeLen
        ...
        elseif PDSCH_DMRS_MAPPING_TYPE == 'type B'   % l  is defined relative to the start of the scheduled PDSCH resources
         ...
         for p = 1:RANK  % no need to execute p=1:PortNum         
            for n=0:nMax
...
%DMRSMatrix = DMRSMatrixAll(:,:,1:RANK);  % pick the first RANK ports
CDMGroupMap = CDMGroup(1:RANK);
end