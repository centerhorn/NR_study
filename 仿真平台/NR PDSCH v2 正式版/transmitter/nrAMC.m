...
function [TBS,QmF,RF] = nrAMC(CQIIndex,LayerNum,pdschConfig)
...
%% 确定每一阶MCS相对应的TBS
TBSTemp = [];
for MCSIndex = 1:size(MCSTablePDSCH,2)
    ...
    if NInfo<=3824
        ...
        TBSTemp(MCSIndex) = TBSTableLess3824(TempIndex);
    else
        n = floor(log2(NInfo-24))-5;
        NInfoTemp = 2^n*round((NInfo-24)/2^n);
        if R<=1/4
           C = ceil((NInfoTemp+24)/3816);
           TBSTemp(MCSIndex) = 8*C*ceil((NInfoTemp+24)/8/C)-24; 
        else
            if NInfoTemp>8424
                C = ceil((NInfoTemp+24)/8424);
                TBSTemp(MCSIndex) = 8*C*ceil((NInfoTemp+24)/8/C)-24;   
            else
                TBSTemp(MCSIndex)  = 8*ceil((NInfoTemp+24)/8)-24;
            end
        end 
    end
end

TBSSE = TBSTemp/DATA_NUM/LayerNum;

%% 确定MCS阶数
SE = CQISETable(CQIIndex);
MCSSEMin = min(TBSSE);
MCSSEMax = max(TBSSE);
if SE<MCSSEMin
   SEMax =  MCSSEMin;
...
MCSTotal = find(TBSSE<=SEMax);
MCSInd = max(MCSTotal);%此处MCSIndex为传统意义上的MCS index加1

RF = MCSTablePDSCH(2,MCSInd);
QmF = MCSTablePDSCH(1,MCSInd);
TBS = TBSTemp(MCSInd);
end