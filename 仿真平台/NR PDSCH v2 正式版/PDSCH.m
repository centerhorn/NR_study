...
for snrIdx = 1:numel(SNRIn)
   ...
   % Initialize  PMI parameter
    ...
    while  nslot < NSlots
        ...
        % Data Precoding
        symbols = nrDLPrecode(PrecodingMatrix,layered); %#OK
        % DMRS Precoding
        DMRSMatrixAfterPrecode = nrDLPrecodeDMRS(PrecodingMatrix,DMRSMatrix); % #OK
        % Resource Mapping
        ...
        if pdsch.IdealChanEstEnabled== 1                                 %%not finished!!!
            ...
            HIdeal = nrPerfectChannelEstimate(pdsch,gnb,channel,H,PrecodingMatrix,RSMapMatrix);
